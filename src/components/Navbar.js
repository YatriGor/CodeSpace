...
function Navbar({ 
  ...
  const saveProjectName = () => {
    setIsEditing(false);
    if (projectName.trim()) {
      onProjectNameChange(projectName.trim(), true);
    }
  };
    ...
    <nav className="bg-[hsl(225,6%,13%)] text-white py-3 px-4 flex justify-between items-center w-full z-50 shadow-md">
      ...
      <div className="flex items-center gap-6">
        {showNewProjectButton && (
          <Link to="/Dashboard">
            <button className="px-4 py-2 gradient-border hover:from-pink-600 hover:to-purple-500 hover:shadow-[0_0_15px_rgba(255,105,180,0.3)] border border-pink-600">
              + New Project
            </button>
          </Link>
        )}
